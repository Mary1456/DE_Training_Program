...
def create_product_performance_summary(spark, silver_base_path, gold_base_path, processing_date_str):
    ...
    try:
        ...
        # No 'status' column in schema, so no filter on status
        # Also, 'total_price' is the correct column for sales amount
        ...
        df_product_summary = (
            ...
            .withColumn("report_date", lit(processing_date_str))  # optional: add report date column
        )
